...
function travel!(preorder::Vector{String}, inorder::Vector{String},pre_index::Vector{Int},left_index::Int,right_index::Int)
    if left_index > right_index || last(pre_index) > length(preorder) || left_index ≤ 0
        return nothing
    end
    root_val = preorder[last(pre_index)]
    push!(pre_index,last(pre_index)+1)
    #pre_index += 1
