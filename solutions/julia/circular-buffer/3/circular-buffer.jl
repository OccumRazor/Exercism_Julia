...
function Base.push!(cb::CircularBuffer, item; overwrite::Bool=false)
    ...
    elseif length(cb) < cb.slots
        ...
        if overwrite
            push!(cb.buffer,item)
            cb.buffer = cb.buffer[2:cb.slots+1]
        else
...
function Base.pushfirst!(cb::CircularBuffer, item; overwrite::Bool=false)
    pushfirst!(cb.buffer,item)
    while length(cb) > cb.slots
        pop!(cb.buffer)
    end
end
...
function Base.append!(cb::CircularBuffer, items::UnitRange; overwrite::Bool=false)
    if length(cb) + length(items) ≤ capacity(cb)
        append!(cb.buffer,items)
    else
        cb.old_id = 1
        cb.last_id = capacity(cb)
        end_id = length(items)
        start_id = end_id - capacity(cb) + 1
        cb.buffer = collect(items)[start_id:end_id]
    end
end
